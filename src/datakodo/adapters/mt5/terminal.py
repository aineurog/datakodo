...
(design doc sec 10), so requests are shifted into server time and returned
...
class MT5Terminal:
    ...
    def __init__(self, terminal_path: str = "", mt5_config: MT5Config | None = None) -> None:
        ...
        # Conservative token bucket around data requests (design doc sec 17).
        # MT5 throttles symbol/data requests; see MT5Config.rate_limit_*.
    ...
    def symbol_info_tick(self, symbol: str) -> Any:
        ...
        — the live-price inputs used for fundamentals.
        """
        if not self._connected:
            raise ConnectionError("MT5 terminal is not connected.")
        self._acquire(1)
